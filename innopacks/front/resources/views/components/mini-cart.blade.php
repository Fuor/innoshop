    ...
    setup() {
      ...
      const toggleSelectAll = () => {
        const normalIds = cartItems.value
          .filter(item => item.item_type === 'normal' || item.item_type === '')
          .map(item => item.id)
      ...
      const formatCurrency = (value) => {
        // 假设货币符号是 $ 并且保留两位小数
        // 这里需要从 Blade 传递货币符号，或者通过 API 获取
        // 示例：'{{ current_currency()->symbol_left }}' + parseFloat(value).toFixed(2);
        // 或者如果后端返回了格式化后的定制项价格，可以直接使用
      ...
      return {
        ...
        updateSelection,
        toggleSelectAll,
        formatCurrency // 暴露 formatCurrency 方法
      }
    }
  }).mount('#miniCart')
}
