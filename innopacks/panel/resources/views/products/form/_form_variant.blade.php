  ...
  <div class="card-body py-0">
    ...
    <div :class="['text-primary add-variant', !variants.length ? 'no-variants' : '']" v-if="variants.length < 3">
      <div class="d-flex align-items-center">
        ...
        {{-- 阶梯价格开关按钮 --}}
        <div v-if="isLadderPricePluginEnabled" class="form-check form-switch d-inline-block ms-5">
          <div class="d-flex align-items-center">
            <input class="form-check-input" type="checkbox" id="toggleLadderPrice" v-model="showLadderPriceEditor">
            <label class="form-check-label" for="toggleLadderPrice">{{ __('LadderPrice::panel.enable_ladder_price') }}</label>
          </div>
        </div>
      </div>
    </div>
    <div class="variant-skus-wrap" v-if="smallVariants.length">
      <div class="variant-skus-table table-responsive">
        <table class="table align-middle">
          ...
          <tbody>
          <template v-for="(sku, index) in skus" :key="index">
            <tr>
              <td>
            ...
            <!-- 阶梯价格编辑区域 - 放在独立的下一行 -->
            <tr class="ladder-price-row" v-if="showLadderPriceEditor">
              <td colspan="6"> {{-- 跨越所有列 --}}
                <div class="ladder-price-editor card card-body bg-light p-3"> {{-- 添加 card 样式使其更明显 --}}
                  <h6 class="card-title mb-2 text-center">{{ __('LadderPrice::panel.ladder_prices') }}</h6>
                  <div v-for="(rule, ruleIndex) in sku.ladder_prices" :key="ruleIndex" class="row mb-1 align-items-center">
                    <div class="col-md-3">
                      <label class="form-label visually-hidden">{{ __('LadderPrice::panel.min_quantity') }}</label>
                      <input type="number" class="form-control form-control-sm" v-model="rule.min_quantity" placeholder="{{ __('LadderPrice::panel.min_quantity') }}">
                    </div>
                    <div class="col-md-3">
                      <label class="form-label visually-hidden">{{ __('LadderPrice::panel.max_quantity') }}</label>
                      <input type="number" class="form-control form-control-sm" v-model="rule.max_quantity" placeholder="{{ __('LadderPrice::panel.max_quantity') }}">
                    </div>
                    <div class="col-md-3">
                      <label class="form-label visually-hidden">{{ __('LadderPrice::panel.price') }}</label>
                      <input type="number" step="0.01" class="form-control form-control-sm" v-model="rule.price" placeholder="{{ __('LadderPrice::panel.price') }}">
                    </div>
                    <div class="col-md-3">
                      <button type="button" class="btn btn-sm btn-danger" @click="removeLadderPriceRule(sku, ruleIndex)"><i class="bi bi-trash"></i> {{ __('panel/common.delete') }}</button>
                    </div>
                  </div>
                  <div class="text-center">
                    <button type="button" class="btn btn-sm btn-outline-primary mt-2" @click="addLadderPriceRule(sku)"><i class="bi bi-plus-lg"></i> {{ __('LadderPrice::panel.add_rule') }}</button>
                  </div>
                </div>
              </td>
            </tr>
    ...
    let variantsBoxApp = createApp({
      ...
      setup() {
        ...
        const skus = ref((() => {
          ...
          // 确保每个 SKU 都有 ladder_prices 属性
          return rawSkus.map(sku => ({
            ...sku,
            ladder_prices: sku.ladder_prices || [] // 确保 ladder_prices 存在且为数组
          }));
        })());
        ...
        const generateSku = () => {
          ...
          for (let i = 0; i < totalCombinations; i++) {
            ...
            let skuItem = {
              ...
              text: '',
              variants: [],
              ladder_prices: skus.value[i] ? skus.value[i].ladder_prices : [],
            };
        ...
        const addLadderPriceRule = (sku) => {
          sku.ladder_prices.push({ min_quantity: null, max_quantity: null, price: null });
        };

        // Remove a ladder price rule from a specific SKU
        const removeLadderPriceRule = (sku, ruleIndex) => {
          sku.ladder_prices.splice(ruleIndex, 1);
        };

        // 插件启用状态 (假设后端会传递这个变量)
        const isLadderPricePluginEnabled = ref(@json(plugin('LadderPrice')?->checkActive())); // 检查插件是否启用
        // 如果任何一个 SKU 已经有阶梯价格，则默认开启
        const showLadderPriceEditor = ref(skus.value.some(sku => sku.ladder_prices && sku.ladder_prices.length > 0));

        // 监听 showLadderPriceEditor 的变化
        watch(showLadderPriceEditor, (newValue) => {
          if (!newValue) {
            // 如果关闭开关，清空所有 SKU 的 ladder_prices
            skus.value.forEach(sku => {
              sku.ladder_prices = [];
            });
          }
        ...
        return {
          ...
          validateQuantity,
          addLadderPriceRule,
          removeLadderPriceRule,
          isLadderPricePluginEnabled,
          showLadderPriceEditor
        }
      }
    }).mount('#variants-box');
